...
export default async function Dashboard() {
  const now = new Date();
  const [events,upcomingCount,pending,confirmed,declined,failed,actions,messages,sync] = await Promise.all([
  ...
  return <><h1>Operations Dashboard</h1><p className="muted">Confirmation and communication status at a glance.</p><section className="grid">
    {[["Upcoming weddings",upcomingCount],["Awaiting confirmation",pending],["Confirmed",confirmed],["Declined",declined],["Failed delivery",failed]].map(([x,n])=><div className="card" key={x}><div className="muted">{x}</div><div className="metric">{n}</div></div>)}
  </section><h2>Next 10 upcoming weddings</h2><table><thead><tr><th>Event</th><th>Date</th><th>Team</th><th>Status</th></tr></thead><tbody>{events.map(e=><tr key={e.id}><td>{e.name}<div className="muted">{e.venueName??"Venue pending"}</div></td><td>{e.startsAt.toLocaleString()}</td><td>{e.assignments.filter(a=>a.active&&a.person.active).map(a=><div key={a.id}>{a.person.displayName} · {a.role.toLowerCase()}</div>)}</td><td><span className="pill">{e.status}</span></td></tr>)}</tbody></table>
