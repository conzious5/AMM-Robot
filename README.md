...
Because the Quo number is also a normal company phone line, automation is opt-in per message and conversation-aware. The published standalone commands STOP, START, HELP, CONFIRM, DECLINE, SCHEDULE, DETAILS, TIMELINE, LOCATION, HOURS, and PAY are processed deterministically only when the latest outbound Quo message was a tracked reminder or robot response. If Cylina or another person sent the latest outbound text within 72 hours, contractor replies stay silent for human handling; STOP, START, and an explicit `ROBOT:` request are the only overrides. DETAILS, TIMELINE, LOCATION, and HOURS may be followed by an event date. All other texts are stored for human review without an automatic reply or scheduling alert. Robot-generated SMS replies end with `Sent by AMM Robot`. The first reminder text includes the help menu once per contractor. PAY exposes only the fixed standard rate card and mileage formula; individual payouts, invoices, client pricing, billing, taxes, and contract amounts remain blocked before database or model lookup. Scheduling tools expose only the identified contractor's whitelisted ceremony fields. E.164 identities are never merged by similar name.
